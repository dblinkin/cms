...
public class Uploader {
	...
	// 保存路径
	private String savePath = FilePath.IMAGE_UPLOAD_PATH;
	// 文件允许格式
			...
			while (fii.hasNext()) {
				...
				if (!fis.isFormField()) {
					...
					BufferedInputStream in = new BufferedInputStream(
							fis.openStream());
					File file = new File(FilePath.ROOT_PATH + this.url);
	...
	public void uploadBase64(String fieldName) {
		...
		try {
			File outFile = new File(FilePath.ROOT_PATH + this.url);
	...
	private String getFolder(String path) {
		...
		path += "/" + dateStr;
		File dir = new File(FilePath.IMAGE_UPLOAD_REALPATH + "/" + dateStr);
		if (!dir.exists()) {
			try {
				dir.mkdirs();
			} catch (Exception e) {
